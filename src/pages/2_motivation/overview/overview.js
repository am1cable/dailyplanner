...
import { CATEGORIZE_ACTIVITIES, PRIORITIZE_ACTIVITIES_TOP, CONFIDENCE_CHECK } from "../../pageUrls";
...
export const Overview = () => {
    ...
    const renderScheduleTimeline = (schedule, index) =>
        <ScheduleTimelineItem
            ...
            {...schedule}
        />
    const renderActivityListItem = ({ name, priority }, i) => <ListItem key={i}>
        <ListItemAvatar><Avatar className={i < 3 ? classes.top3Icon : ''}>{i + 1}</Avatar></ListItemAvatar>
        <ListItemText primary={name} secondary={priorities[priority]} />
    </ListItem>

    return <PageWrapper className="overview" back={{ link: PRIORITIZE_ACTIVITIES_TOP }} forward={{ link: CONFIDENCE_CHECK }}>
                ...
                <Grid container direction="row" spacing={3}>
                    ...
                    <Grid item xs={8}>
                        <Card variant="outlined">
                            <CardContent>
                                <List>
                                    {activitesInDay.filter(a => a.categoryId === selectedScheduleItem).sort(prioritizedActivities).sort(priorityWeightizedActivities).map(renderActivityListItem)}
                                </List>
                            </CardContent>
                        </Card>
                    </Grid>
